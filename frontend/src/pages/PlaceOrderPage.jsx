...
const PlaceOrderPage = () => {
    ...
    const orderApi = {
            ...
            try {
                ...
                const hydratedItems = await Promise.all(rawItems.map(async (item) => {
                    ...
                    try {
                        ...
                        return { id, qty, name: "Product Unavailable", price: 0, image: "" };
                    }
                }));
        ...
        // 2. PLACE ORDER (FIXED HERE)
        placeOrder: async (fullAddress) => {
            // FIX: Changed key back to 'address' to match the database column name
            ...
            <div className={styles.contentWrapper}>
                        ...
                        {step === 2 && (
                            <div className={styles.stepBody}>
                                <div className={styles.paymentOptions}>
                                    <div className={`${styles.paymentOption} ${paymentMethod === 'Cash on Delivery' ? styles.selectedPayment : ''}`} onClick={() => setPaymentMethod('Cash on Delivery')}>
                                        <Truck size={24} /><span>Cash on Delivery</span>
                                    </div>
                                    <div className={`${styles.paymentOption} ${paymentMethod === 'Credit Card' ? styles.selectedPayment : ''}`} onClick={() => setPaymentMethod('Credit Card')}>
                                        <CreditCard size={24} /><span>Credit Card</span>
                                    </div>
                                </div>
                                ...
                                <div className={styles.reviewItems}>
                                    {cartItems.map(item => (
                                        <div key={item.id} className={styles.reviewItem}>
                                            ...
                                            <div className={styles.reviewPrice}>{formatCurrency(item.qty * item.price)}</div>
                                        </div>
                                    ))}
                                </div>
                            </div>
                ...
                <div className={styles.summarySection}>
                    <div className={styles.summaryCard}>
                        <h3>Order Summary</h3>
                        <div className={styles.summaryRow}><span>Items:</span><span>{formatCurrency(subtotal)}</span></div>
                        <div className={styles.summaryRow}><span>Shipping:</span><span>{formatCurrency(shippingCost)}</span></div>
                        <div className={styles.summaryRow}><span>Tax (14%):</span><span>{formatCurrency(tax)}</span></div>
                        <div className={`${styles.summaryRow} ${styles.totalRow}`}><span>Order Total:</span><span>{formatCurrency(total)}</span></div>
                        <button className={styles.placeOrderBtn} onClick={handlePlaceOrder} disabled={isPlacingOrder || step < 3}>
